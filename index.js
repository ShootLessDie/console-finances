...
let monthCounter = finances.length
let total = 0
let aveChange = 0
let maxProfit = 0
...
for (let i=0; i<finances.length; i++) {
    ...
    /* Set maxLoss variable (greatest decrease in profits) to the smallest figure observed */
        if  ((finances[i][1] - finances[i-1][1]) < maxLoss){
            maxLoss = finances[i][1] - finances[i-1][1]
        }

    /* Set maxProfit variable (greatest increase in profits) to the largest figure observed */
...
Average Change: $${(total/monthCounter).toFixed(2)} or $${(changeSum/change.length).toFixed(2)}
Greatest increase in profits:${finances[maxProfitMonth][0]} ${Math.round(maxProfit)}
Greatest decrease in profits:${finances[maxLossMonth][0]} ${Math.round(maxLoss)} `)
